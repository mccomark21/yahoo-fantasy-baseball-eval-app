...
import type { HitterRankTrendRow } from '@/lib/queries';

type HitterSortKey = 'latest_rank' | 'movement_value' | 'trend_8w_net' | 'player_name' | 'fantasy_team';

interface HitterRankTableProps {
  data: HitterRankTrendRow[];
  isLoading: boolean;
}
...
// "Jun 23" from a 'YYYY-MM-DD' snapshot date, rendered at UTC noon so the cited
// note date never slips a day by the viewer's timezone.
function formatNoteDate(snapshotDate: string | null): string | null {
...
function movementSortValue(row: HitterRankTrendRow): number {
...
function getSortValue(row: HitterRankTrendRow, key: HitterSortKey): number | string {
...
// Pitcher List write-ups are sparse on the hitter board, so a player's note can
// be weeks old. When the note predates the current article we cite when it ran
// so a manager reads it as context, not breaking news.
function NoteCell({ notes, notesDate }: { notes: string | null; notesDate: string | null }) {
  if (!notes) return <>—</>;
  const dateLabel = formatNoteDate(notesDate);
  return (
    <>
      {notes}
      {dateLabel ? (
        <span className="ml-1 whitespace-nowrap text-muted-foreground/70">· noted {dateLabel}</span>
      ) : null}
    </>
  );
}

export function HitterRankTable({ data, isLoading }: HitterRankTableProps) {
  const isMobile = useIsMobile();
  const [sortKey, setSortKey] = useState<HitterSortKey>('latest_rank');
  ...
  const setSort = (key: HitterSortKey) => {
    if (sortKey === key) {
      setSortDesc((v) => !v);
      return;
    }
  ...
  const sortIcon = (key: HitterSortKey) => {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center p-12 text-muted-foreground">Loading hitter rankings...</div>;
  }
              ...
              <div key={rowKey} className="border-b px-3 py-2.5">
                ...
                {isExpanded && hasNote ? (
                  <div className="mt-2 text-xs leading-5 text-muted-foreground">
                    <NoteCell notes={row.notes} notesDate={row.notes_date} />
                  </div>
                ) : null}
              </div>
            );
  ...
  return (
    <div className="flex min-h-0 flex-col flex-1 overflow-hidden">
          ...
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="cursor-pointer select-none" onClick={() => setSort('player_name')}>
                <div className="flex items-center gap-1">Hitter {sortIcon('player_name')}</div>
              </TableHead>
      ...
      <div className="border-t px-4 py-2 text-sm text-muted-foreground">Showing {sortedData.length} hitters</div>
    </div>
  );
}
