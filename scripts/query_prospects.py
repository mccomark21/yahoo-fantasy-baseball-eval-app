...
hitters_with_stats = df[df["atBats"].notna()][
    ["player_name", "org", "level", "atBats", "avg", "homeRuns", "rbi", "ops", "stats_summary"]
].drop_duplicates(subset=["player_name", "org"]).head(5)
for _, row in hitters_with_stats.iterrows():
...
pitchers_with_stats = df[df["inningsPitched"].notna()][
    ["player_name", "org", "level", "inningsPitched", "era", "strikeoutsPer9Inn", "whip", "stats_summary"]
].drop_duplicates(subset=["player_name", "org"]).head(5)
for _, row in pitchers_with_stats.iterrows():
    print(f"\n{row['player_name']} ({row['org']}) - {row['level']}")
...
print(
    f"Prospects with Fangraphs rank: {df['fangraphs_rank'].notna().sum()} ({(df['fangraphs_rank'].notna().sum() / len(df) * 100):.1f}%)"
)
